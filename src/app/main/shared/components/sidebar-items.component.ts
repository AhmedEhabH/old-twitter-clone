...
@Component({
  ...
  template: `
    ...
    <div class="flex flex-row items-center rounded-full hover:bg-slate-300 hover:bg-opacity-10 cursor-pointer" *ngFor="let item of items">
      <a [routerLink]="item.route"
      [ngClass]="{'text-white':!item.isActive, 'text-sky-500':item.isActive}"
      class="material-icons relative h-14 w-14 flex items-center justify-center p-4">
        {{item.icon}}
      </a>
      <a routerLink="item.route" class="relative hidden lg:flex items-center gap-4 p-4">
        <p class="hidden lg:block text-white text-lg">{{item.label}}</p>
      </a>
    </div>

    <!-- Logout Button -->
    <div class="flex flex-row items-center rounded-full hover:bg-opacity-10 hover:bg-slate-300 cursor-pointer">
      <div class="material-icons text-white relative h-14 w-14 flex items-center justify-center p-4">
        logout
      </div>
      <div class="relative hidden lg:flex items-center gap-4 p-4 ">
